...
def plot_training_curve(history, L2_value):
    ...
    plt.savefig(f'training_curves_model2{L2_value}.pdf')
...
def train_cnn(L2_value):
    ...
    model.add(Conv2D(16, (11, 11), strides=(4, 4), padding='same', activation='relu', input_shape=input_shape, kernel_initializer=he_normal(), kernel_regularizer=l2(L2_value)))
    model.add(MaxPooling2D((3, 3), strides=(2, 2)))

    # Segunda capa convolucional + Max Pooling + Dropout
    model.add(Conv2D(32, (5, 5), padding='same', activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(L2_value)))
    model.add(MaxPooling2D((3, 3), strides=(2, 2)))

    # Cuarta capa convolucional
    model.add(Conv2D(64, (3, 3), padding='same', activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(L2_value)))

    # Quinta capa convolucional + Max Pooling + Dropout
    model.add(Conv2D(32, (3, 3), padding='same', activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(L2_value)))
    model.add(MaxPooling2D((3, 3), strides=(2, 2)))

    # Aplanar la salida
    model.add(Flatten())

    # Capa completamente conectada con Dropout
    model.add(Dense(256, activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(L2_value)))

    # Capa de salida
    model.add(Dense(29, activation='softmax', kernel_initializer=he_normal(), kernel_regularizer=l2(L2_value)))
    ...
    plot_training_curve(history, L2_value)
...
if __name__ == "__main__":
    ...
    for L2_value in L2_values:
        print(f"Training with Dropout: {L2_value}")
        train_cnn(L2_value)
