...
def train_cnn():
    ...
    # Definición del modelo LeNet-5 adaptado
    model = Sequential()

    # Primera capa convolucional y de pooling (equivalente a C1 y S2 en LeNet-5)
    model.add(Conv2D(6, (5, 5), activation='relu', padding='valid',
                     input_shape=input_shape, kernel_initializer=he_normal()))
    model.add(AveragePooling2D(pool_size=(2, 2), strides=(2, 2)))

    # Segunda capa convolucional y de pooling (equivalente a C3 y S4 en LeNet-5)
    model.add(Conv2D(16, (5, 5), activation='relu',
                     padding='valid', kernel_initializer=he_normal()))
    model.add(AveragePooling2D(pool_size=(2, 2), strides=(2, 2)))

    # Tercera capa convolucional (equivalente a C5 en LeNet-5)
    # Reducimos el número de filtros y ajustamos el tamaño del filtro
    ...
    # Primera capa completamente conectada (equivalente a F6 en LeNet-5)
    # Reducimos la cantidad de neuronas
    model.add(Dense(64, activation='relu', kernel_initializer=he_normal()))

    # Capa de salida con softmax para 29 clases
