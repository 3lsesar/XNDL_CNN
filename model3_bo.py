...
def train_cnn():
    ...
    model.add(Conv2D(16, (11, 11), strides=(4, 4), padding='same',
                     activation='relu', input_shape=input_shape, kernel_initializer=he_normal(), kernel_regularizer=l2(0.001)))
    model.add(MaxPooling2D((3, 3), strides=(2, 2)))

    # Segunda capa convolucional + Max Pooling + Dropout
    model.add(Conv2D(32, (5, 5), padding='same',
                     activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(0.001)))
    model.add(MaxPooling2D((3, 3), strides=(2, 2)))
    model.add(Dropout(0.3))  # Aplicar dropout aquí
    ...
    model.add(Dropout(0.3+0.1))  # Aplicar un dropout
