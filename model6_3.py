...
def train_cnn():
    ...
    model.add(Conv2D(256, (3, 3), padding='same', activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(0.0001)))  # Reducido el número de filtros y ajustado L2
    model.add(BatchNormalization())
    model.add(Conv2D(256, (3, 3), padding='same', activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(0.0001)))  # Reducido el número de filtros y ajustado L2
    model.add(BatchNormalization())
    model.add(tf.keras.layers.MaxPooling2D((2, 2), strides=(2, 2)))
    ...
    learning_rate = 0.01  # Ajustado la tasa de aprendizaje
    mom = 0.4
