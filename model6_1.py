...
def train_cnn():
    ...
    model.add(Conv2D(256, (3, 3), padding='same', activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(0.001)))  # Reducido el número de filtros y ajustado L2
    model.add(BatchNormalization())
    model.add(Conv2D(256, (3, 3), padding='same', activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(0.001)))  # Reducido el número de filtros y ajustado L2
    model.add(BatchNormalization())
    model.add(tf.keras.layers.MaxPooling2D((2, 2), strides=(2, 2)))
    ...
    model.add(BatchNormalization())
    model.add(Dropout(0.5))  # Aumentado el Dropout
    model.add(Dense(29, activation='softmax', kernel_initializer=he_normal(), kernel_regularizer=l2(0.01)))

    # Elegir optimizador y compilar el modelo
    learning_rate = 0.001  # Ajustado la tasa de aprendizaje
    mom = 0.4
