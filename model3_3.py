...
def plot_training_curve(history, dropout_value, L2_value):
    ...
    plt.savefig(f'training_curves_model2{dropout_value}{L2_value}.pdf')
...
def train_cnn(dropout_value, L2_value):
    ...
    model.add(Conv2D(16, (11, 11), strides=(4, 4), padding='same',
                     activation='relu', input_shape=input_shape, kernel_initializer=he_normal(), kernel_regularizer=l2(L2_value)))
    model.add(MaxPooling2D((3, 3), strides=(2, 2)))

    # Segunda capa convolucional + Max Pooling + Dropout
    model.add(Conv2D(32, (5, 5), padding='same',
                     activation='relu', kernel_initializer=he_normal(), kernel_regularizer=l2(L2_value)))
    model.add(MaxPooling2D((3, 3), strides=(2, 2)))
    model.add(Dropout(dropout_value))  # Aplicar dropout aquí
    ...
    model.add(Dropout(dropout_value+0.1))  # Aplicar un dropout
    ...
    print(f"ITERACO: {dropout_value} {L2_value}")
    # Check model summary!
    print(model.summary())
    ...
    plot_training_curve(history, dropout_value, L2_value)
...
if __name__ == "__main__":
    ...
    for dropout_value in dropout_values:
        for L2_value in L2_values:
            print(f"Training with Dropout: {dropout_value}")
            train_cnn(dropout_value, L2_value)
